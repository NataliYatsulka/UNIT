...
t_list_1	*ft_fd(const int fd, t_list_1 *list)
{
	if (list == NULL)
	{
...
char		*check_2(char *cut, t_list_1 *list)
{
	char			*t;
...
int			ft_int(t_list_1 *list, char **line)
{
	if ((list->str)[0] == '\0')
		return (0);
	*line = list->str;
...
int			get_next_line(const int fd, char **line)
{
	...
	if (fd < 0 || !list || !list->str || !(line) ||
		...
		return (-1);
	list->str = check_2(list->cut, list);
